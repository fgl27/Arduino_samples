...
void setup () {
  ...
  PWM->PWM_CH_NUM[6].PWM_CMR =  PWM_CMR_DTE | PWM_CMR_CPRE_CLKA; //| PWM_CMR_CALG | PWM_CMR_CPOL
  PWM->PWM_CH_NUM[6].PWM_CPRD = PERIOD;                               // Set the PWM period register 84MHz/(40kHz)=2100;
  PWM->PWM_CH_NUM[6].PWM_DT = PWM_DT_DTH(0) | PWM_DT_DTL(PERIOD / 4);
  PWM->PWM_CH_NUM[6].PWM_CDTY = 0;

  PWM->PWM_CH_NUM[4].PWM_CMR =  PWM_CMR_CPRE_CLKA | PWM_CMR_DTE; //| PWM_CMR_CALG | PWM_CMR_CPOL
  PWM->PWM_CH_NUM[4].PWM_CPRD = PERIOD;                               // Set the PWM period register 84MHz/(40kHz)=2100;
  PWM->PWM_CH_NUM[4].PWM_DT = PWM_DT_DTH(0) | PWM_DT_DTL(PERIOD / 2);
  PWM->PWM_CH_NUM[4].PWM_CDTY = 0;

  PWM->PWM_CH_NUM[5].PWM_CMR =  PWM_CMR_CPRE_CLKA | PWM_CMR_CPOL; //| PWM_CMR_CALG | PWM_CMR_CPOL
  PWM->PWM_CH_NUM[5].PWM_CPRD = PERIOD;                               // Set the PWM period register 84MHz/(40kHz)=2100;
  PWM->PWM_CH_NUM[5].PWM_CDTY = 0;
  
  REG_PWM_ENA = PWM_ENA_CHID7 | PWM_ENA_CHID6 | PWM_ENA_CHID5 | PWM_ENA_CHID4; //| PWM_ENA_CHID6 | PWM_ENA_CHID5 |
}

void loop() {
  DUTY++;
  if (DUTY > (PERIOD / 4)) DUTY = 0;
  PWM->PWM_CH_NUM[7].PWM_CDTYUPD = DUTY;
  PWM->PWM_CH_NUM[6].PWM_CDTYUPD = DUTY + (PERIOD / 4);
  PWM->PWM_CH_NUM[5].PWM_CDTYUPD = PERIOD - DUTY;
  PWM->PWM_CH_NUM[4].PWM_CDTYUPD = DUTY + (PERIOD / 2);
  //Serial.println(DUTY);
  //Serial.print("volts = ");
